...
- [x] **[Moz+agent] Prove the upload command** (25 Sep 2026: `deploy.sh`; rollback copy of the 2020 web root taken first) on the first deploy (rsync with sudo on the remote end and the right ownership), then add a `deploy` npm script. Take a copy of the current `www/default/` into the scratchpad first as the rollback.
...
- [x] **[Moz+agent] Create `work@mozmorris.co.uk`** (Moz: already set up, 25 Sep 2026) on the Mail-in-a-Box (mailbox or alias) and send a test message before the page goes live.
